...
# Concerns
- The background watermark should be the ecopoints logo, not the text "ECOPOINTS". Specifically ![alt text](client/public/ecopoints-primary-logo.png)
- The machine shouldn't tilt when hovered. It should emphasize by getting bigger by a scale of 1.1.
- The annotations should only be emphasized if hovered, not with the machine.
- Move the other annotation "Accessible for the Community" on the left side so container "Verification..." would not overlap it.
- Change the floating modal "Verification..." content. Make the content a notification type where in the user's points are added on their account.
...
Scope labels are attached as the first sub-bullet of every remaining leaf task to mark whether it is part of the current "fix the website + admin dashboard" priority or deferred until RVM hardware work resumes:

- **`Scope: web-required`** — required for the website / admin dashboard hardening priority. Execute now.
- **`Scope: web-required (rpi-carveout)`** — required, but the task references RPi-only artifacts that should be skipped or stubbed until Phase 4A lands. Notes on the carve-out are inline with the task.
- **`Scope: rpi-deferred`** — RPi / RVM hardware refactoring. Defer until the website + admin work is complete.

## Tasks

- [x] 1. Phase 0 — Critical Security Hotfixes (close GET-bypass)
  - [x] 1.1 Introduce shared admin guard helper in `server/app/middleware.py`
  ...
  - [x] 1.2 Refactor `@admin_required` to call `_require_admin_or_403` first
  ...
  - [x] 1.3 Refactor `@permission_required(*categories)` to call `_require_admin_or_403` first
    - Remove the `if request.method != 'GET'` early-return branch entirely.
  ...
  - [x] 1.4 Sanitize `ROLE_PERMISSIONS` to remove non-admin entries
  ...
  - [x] 1.5 Write property test for the universal admin guard
  ...
  - [x] 1.6 Write static-analysis test for decorator stacking
...
- [x] 2. Checkpoint — Phase 0 closure
...
- [x] 3. Phase 1 (Server) — Decompose `web_controller.py` into Domain_Controllers
  - [x] 3.1 Create `server/app/controllers/_shared.py` with serializer and helper utilities
  ...
  - [x] 3.2 Extract `dashboard_controller.py`
  ...
  - [x] 3.3 Extract `users_controller.py`
  ...
  - [x] 3.4 Extract `locations_controller.py`
  ...
  - [x] 3.5 Extract `machines_controller.py`
  ...
  - [x] 3.12 Extract `sessions_controller.py`
  ...
  - [x] 3.13 Reduce `web_controller.py` and register sub-blueprints in `server/app/__init__.py`
  ...
  - [x] 3.14 Write property tests for backward-compatible paths and decorator preservation
...
- [x] 4. Phase 1 (Client) — Decompose `apiService.js` into `client/src/services/api/`
  - [x] 4.1 Create `client/src/services/api/client.js` (single source of `request()`)
  ...
  - [x] 4.2 Create per-domain modules under `client/src/services/api/` and an `index.js`
  ...
  - [x] 4.3 Delete `client/src/services/apiService.js`, drop `cities`, and rewrite all imports
  ...
  - [x] 4.4 Write static tests for client API hygiene
...
- [x] 5. Checkpoint — Phase 1 closure
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 6. Phase 2 — RBAC enforcement on routes and Admin_UI page guards
  - [x] 6.1 Define authoritative `ROLE_PERMISSIONS` map in `middleware.py`
  ...
  - [x] 6.2 Substitute `@permission_required(...)` for every `@admin_required` across Domain_Controllers
  ...
  - [x] 6.3 Extend `GET /api/web/auth/me` to return `role` and `permission_categories`
    - Compute `permission_categories = sorted(ROLE_PERMISSIONS.get(user.role, []))` and add it to the response body alongside `role`.
    - _Requirements: 2.3_

  - [x] 6.4 Implement `RequirePermission` guard in `client/src/components/admin/RequirePermission.jsx`
  ...
  - [x] 6.5 Wrap every `client/app/admin/**/page.js` with `RequirePermission`
  ...
  - [x] 6.6 Filter Admin_UI sidebar links by `permission_categories`
  ...
  - [x] 6.7 Enforce Role_Hierarchy on user-create and user-update controllers
  ...
  - [x] 6.8 Add `log_action()` helper to `_shared.py` and wire it into mutating handlers
  ...
  - [x] 6.9 Write property test for admin granularity enforcement
  ...
  - [x] 6.10 Write property test for Admin_UI page guard completeness
  ...
  - [x] 6.11 Write property test for audit log completeness
...
- [x] 7. Checkpoint — Phase 2 closure
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 8. Phase 3 — Align Server JSON shapes with Admin_UI fields
  - [x] 8.1 Author `docs/model-ui-alignment.md`
  ...
  - [x] 8.2 Update serializers in `server/app/controllers/_shared.py` per the alignment doc
  ...
  - [x] 8.3 Update Admin_UI pages to use exact field names with empty-state fallbacks
  ...
  - [x] 8.4 Document typed JSON schemas for every aligned endpoint in `api_routes_documentation.md`
  ...
  - [x] 8.5 Write property test for page–field coverage
  ...
  - [x] 8.6 Write property test for strict-acceptance on mutating endpoints
...
- [x] 9. Checkpoint — Phase 3 closure
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 10. Phase 4A — RPI Authentication, HMAC QR, and Hardware Contract
  - **Scope: rpi-deferred** — completed 2026-05-27.
  - [x] 10.1 Create Flask-Migrate revision `phase4a_rpi_auth`
    - **Scope: rpi-deferred**
  ...
  - [x] 10.2 Update `RVM` model with `api_key_hash` and `verify_api_key`
    - **Scope: rpi-deferred**
  ...
  - [x] 10.3 Update `Organization` model with `qr_hmac_secret_enc` and `get_qr_hmac_secret`
    - **Scope: rpi-deferred**
  ...
  - [x] 10.4 Implement RVM provisioning flow (server-side key generation)
    - **Scope: rpi-deferred**
  ...
  - [x] 10.5 Implement `@rpi_auth_required` decorator in `middleware.py`
    - **Scope: rpi-deferred**
  ...
  - [x] 10.6 Apply `@rpi_auth_required` to every route in `rpi_controller.py`
    - **Scope: rpi-deferred**
    - Decorate every route under `/api/rpi/*`; the only exception is an optional `/api/rpi/health` if it exists.
    - _Requirements: 4A.3, 4A.8_

  - [x] 10.7 Implement HMAC-suffix validation in `POST /api/rpi/authenticate`
    - **Scope: rpi-deferred**
  ...
  - [x] 10.8 Author `docs/rpi-api-contract.md`
    - **Scope: rpi-deferred**
  ...
  - [x] 10.9 Write property test for universal RPI auth
    - **Scope: rpi-deferred**
  ...
  - [x] 10.10 Write property test for HMAC-QR round-trip and short-circuit
    - **Scope: rpi-deferred**
...
- [x] 11. Phase 4B — JWT in HttpOnly cookie + CSRF
  - **Scope: web-required** — closes the localStorage-JWT and CSRF gaps for the Client + admin dashboard.
  - [x] 11.1 Set HttpOnly token cookie and CSRF cookie on successful login
    - **Scope: web-required**
  ...
  - [x] 11.2 Update `@token_required` to read cookie first, fall back to `Authorization: Bearer`
    - **Scope: web-required**
  ...
  - [x] 11.3 Implement and apply `@csrf_required`
    - **Scope: web-required**
  ...
  - [x] 11.6 Read `csrf_token` cookie and attach `X-CSRF-Token` on state-changing requests
    - **Scope: web-required**
  ...
  - [x] 11.7 Write property test for cookie + CSRF transport
    - **Scope: web-required**
  ...
  - [x] 11.8 Write property test for cookie-vs-Bearer transition behavior
    - **Scope: web-required**
  ...
  - [x] 11.9 Write static test for JWT-not-in-localStorage invariant
    - **Scope: web-required**
...
- [x] 12. Phase 4C — Force-logout invariant
  - **Scope: web-required** — admin Settings → Security force-logout button.
  - [x] 12.1 Create Flask-Migrate revision `phase4c_force_logout`
    - **Scope: web-required**
  ...
  - [x] 12.2 Update `Organization` model with `force_logout_at`
    - **Scope: web-required**
    - Add the column to the SQLAlchemy model.
    - _Requirements: 4C.17_

  - [x] 12.3 Implement `POST /api/web/settings/security/force-logout`
    - **Scope: web-required**
  ...
  - [x] 12.4 Reject JWTs older than `force_logout_at` in `@token_required`
    - **Scope: web-required**
  ...
  - [x] 12.5 Write property test for forced-logout invariant
    - **Scope: web-required**
...
- [x] 13. Phase 4D — Security headers at the nginx edge
  - **Scope: web-required** — protects every browser response from the website + admin dashboard.
  - [x] 13.1 Add Security_Headers to `nginx/default.conf`
    - **Scope: web-required**
  ...
  - [x] 13.2 Write integration test asserting headers are present on responses
    - **Scope: web-required**
...
- [x] 14. Phase 4E — Schema-validated input on every mutating route
  - **Scope: web-required (rpi-carveout)** — validate auth + Domain_Controller routes now; RPi schemas (`rpi_controller`) deferred until Phase 4A.
  - [x] 14.1 Create `server/app/schemas/__init__.py` with one Pydantic v2 schema per request body
    - **Scope: web-required (rpi-carveout)** — author schemas for `auth_controller` + every Domain_Controller. Skip `rpi_controller` schemas; revisit in Phase 4A.
  ...
  - [x] 14.2 Implement `@validate_request(Schema)` decorator in `middleware.py`
    - **Scope: web-required**
  ...
  - [x] 14.3 Apply `@validate_request` to every POST/PUT/PATCH handler in every controller
    - **Scope: web-required (rpi-carveout)** — apply across `auth_controller` + every Domain_Controller. Skip `rpi_controller` handlers until Phase 4A.
  ...
  - [x] 14.4 Write property test for schema validation completeness
    - **Scope: web-required (rpi-carveout)** — assert the invariant for every web POST/PUT/PATCH route. Restrict the route enumeration to `web_bp` (and `auth_bp`) until RPi schemas land.
...
- [x] 15. Phase 4F — Email HTML escape
  - **Scope: web-required** — admin / user notification emails are rendered server-side; XSS escape protects all admin-dashboard-driven mailings.
  - [x] 15.1 Wrap every user-supplied interpolation in `notification_service._build_email_html` with `html.escape(..., quote=True)`
    - **Scope: web-required**
  ...
  - [x] 15.2 Write property test for email output escaping
    - **Scope: web-required**
...
- [x] 16. Phase 4G — Password policy on admin-created users
  - **Scope: web-required** — admin Users page → Create/Edit user modal.
  - [x] 16.1 Extract `validate_password_policy(password)` into a shared module
    - **Scope: web-required**
  ...
  - [x] 16.2 Apply `validate_password_policy` in `users_controller.create_user`
    - **Scope: web-required**
    - Call before any DB write; on failure return HTTP 400 `{ error: { code: "WEAK_PASSWORD", policy: <message> } }`.
    - _Requirements: 4G.28, 4G.29_

  - [x] 16.3 Write property test for password policy on admin-create
    - **Scope: web-required**
...
- [x] 17. Phase 4H — Role-Hierarchy on PUT user
  - **Scope: web-required** — admin Users / Permissions pages.
  - [x] 17.1 Extend hierarchy enforcement to `PUT /api/web/users/<id>`
    - **Scope: web-required**
  ...
  - [x] 17.2 Write property test for hierarchy on update
    - **Scope: web-required**
...
- [x] 18. Phase 4I — Token blacklist cleanup job
  - **Scope: web-required** — keeps the Auth blacklist bounded; admin Sessions / Logout health depends on it.
  - [x] 18.1 Implement `flask cleanup-tokens` CLI command
    - **Scope: web-required**
  ...
  - [x] 18.2 Document the schedule in `server/README.md`
    - **Scope: web-required**
  ...
  - [x] 18.3 Write property test for bounded token blacklist
    - **Scope: web-required**
...
- [x] 19. Checkpoint — Phase 4 closure
  - **Scope: web-required (rpi-carveout)** — close 4B/4C/4D/4E/4F/4G/4H/4I; record Phase 4A as `deferred` rather than `closed` in `docs/phase-status.md`. Drop the `docs/rpi-api-contract.md` exit criterion until Phase 4A resumes.
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 20. Phase 5 — Deterministic seed and login redirects
  - **Scope: web-required (rpi-carveout)** — login redirect + per-role users are website work; the RVM provisioning step inside the seeder is `rpi-deferred`.
  - [x] 20.1 Rewrite `server/app/seeder/seed.py` to satisfy the deterministic seed contract
    - **Scope: web-required (rpi-carveout)** — keep the OrgType / Organization / CommunityGroup / Reward / per-role User parts and the password-policy gate. Skip the RVM API-key + `qr_hmac_secret_enc` seeding (depends on Phase 4A columns); seed an RVM row without those columns, or omit the RVM entirely until Phase 4A.
  ...
  - [x] 20.2 Implement role-based post-login redirect in `LogIn.jsx`
    - **Scope: web-required**
  ...
  - [x] 20.3 Write property test for deterministic seed
    - **Scope: web-required (rpi-carveout)** — relax the RVM `api_key_hash`-non-null assertion to match the carve-out chosen in 20.1; reinstate it once Phase 4A lands.
  ...
  - [x] 20.4 Write property test for seed password policy
    - **Scope: web-required**
  ...
  - [x] 20.5 Write property test for login redirect
    - **Scope: web-required**
...
- [x] 21. Cross-phase non-functional verification
  - **Scope: web-required (rpi-carveout)** — startup checks, ledger, secret hygiene, and migration reversibility apply now; drop the `qr_hmac_secret_ref` element from the secret set and exclude `phase4a_rpi_auth` from the migration round-trip until Phase 4A.
  - [x] 21.1 Add startup-time secret-presence check in `server/app/__init__.py`
    - **Scope: web-required (rpi-carveout)** — required-secret set is `{SECRET_KEY, DATABASE_URL, SMTP password, SMS provider key}` for now; add `qr_hmac_secret_ref` once Phase 4A lands.
  ...
  - [x] 21.2 Initialize and maintain `docs/phase-status.md`
    - **Scope: web-required**
  ...
  - [x] 21.3 Write static-scan property test for secret hygiene
    - **Scope: web-required**
  ...
  - [x] 21.4 Write property test for production secret refusal
    - **Scope: web-required (rpi-carveout)** — match the secret set chosen in 21.1; reinstate the `qr_hmac_secret_ref` case once Phase 4A lands.  
  ...
  - [x] 21.5 Write integration test for migration reversibility
    - **Scope: web-required (rpi-carveout)** — exercise `phase4c_force_logout` only. Defer `phase4a_rpi_auth` until Phase 4A.
...
- [x] 22. Final checkpoint — Program closure
  - **Scope: web-required (rpi-carveout)** — Phase 4A is intentionally `deferred` (not `closed`) in `docs/phase-status.md`; relax the program-closure assertion to "every non-deferred phase is `closed`" until Phase 4A resumes.
  - Ensure all tests pass, ask the user if questions arise.
...
> **Note (web-first execution).** The wave order below was reorganized to
> prioritize the website + admin dashboard hardening (Phase 4B–4I, Phase 5,
> Cross-phase) before the RPi-deferred Phase 4A work (tasks 10.x). Within
> each phase the original blocking edges are preserved; only the
> cross-phase ordering between Phase 4A and Phase 4B+ was flipped.
> Carve-outs documented in `docs/rpi-revisit-checklist.md` MUST be
> revisited when Phase 4A re-enters the queue.
...
```json
{
  "waves": [
    ...
    { "id": 14, "tasks": ["8.5", "8.6"] },
    { "id": 15, "tasks": ["11.1", "11.2", "11.4"] },
    { "id": 16, "tasks": ["11.3", "11.5"] },
    { "id": 17, "tasks": ["11.6"] },
    { "id": 18, "tasks": ["11.7", "11.8", "11.9"] },
    { "id": 19, "tasks": ["12.1"] },
    { "id": 20, "tasks": ["12.2"] },
    ...
    { "id": 23, "tasks": ["13.1"] },
    { "id": 24, "tasks": ["13.2"] },
    { "id": 25, "tasks": ["14.1", "14.2"] },
    { "id": 26, "tasks": ["14.3"] },
    { "id": 27, "tasks": ["14.4", "15.1", "16.1"] },
    { "id": 28, "tasks": ["15.2", "16.2", "17.1", "18.1"] },
    { "id": 29, "tasks": ["16.3", "17.2", "18.2", "18.3"] },
    { "id": 30, "tasks": ["20.1", "20.2", "21.1", "21.2"] },
    { "id": 31, "tasks": ["20.3", "20.4", "20.5", "21.3", "21.4", "21.5"] },
    { "id": 32, "tasks": ["10.1"] },
    { "id": 33, "tasks": ["10.2", "10.3"] },
    { "id": 34, "tasks": ["10.4", "10.5"] },
    { "id": 35, "tasks": ["10.6", "10.7", "10.8"] },
    { "id": 36, "tasks": ["10.9", "10.10"] }
  ]
}
```

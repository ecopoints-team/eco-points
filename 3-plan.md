...
# Concerns
- In terms of responsiveness in screen size, it doesn't adjust well. Even though it adjusts on mobile devices (wherein it goes to a single column layout and hides the machine area). On other scales, it doesn't adjust nicely.
   - The annotations get cut off if the screen is too small
   - We should still include the machine after scrolling if it is on mobile. Make it single column layout wherein the first column occupies the whole screen and when scrolled, only can the second column (machine) be seen.
   - On 100% display scale, it looks normal. But with the recommended (125%) display scale, it's not adjusting well.
   - The fonts should also adjust on the responsiveness on both zoom in and out and screen size.
...
export default function LogIn({ onClose }) {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
      {/* 2. THE MODAL BOX */}
      <div className="primary-color border border-lime-500/30 p-8 rounded-2xl shadow-2xl w-full max-w-md relative">
        {/* Close Button (X) */}
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          <div>
            <label className="text-sm text-white font-bold ml-1">
              Username
            </label>
            <input
              type="text"
        ...
        <p className="text-xs text-center text-gray-300 mt-4">
          (Prototype Hint: User: <strong>admin</strong> | Pass:{" "}
